...
import net.sf.gazpachoquest.rest.shiro.APIKeyRealm;
import net.sf.gazpachoquest.rest.shiro.APIKeyToken;
...
@DatabaseSetup("ShiroHelloWorld-dataset.xml")
@DbUnitConfiguration(dataSetLoader = ColumnDetectorXmlDataSetLoader.class)
public class ShiroHelloWorld {

    @Autowired
    private APIKeyRealm apiKeyRealm;

    @Test
    public void helloShiro() {
        // instantiate or acquire a Realm instance. We'll discuss Realms later.

        SecurityManager securityManager = new DefaultSecurityManager(apiKeyRealm);

        // Make the SecurityManager instance available to the entire application via static memory:
        SecurityUtils.setSecurityManager(securityManager);

        Subject subject = SecurityUtils.getSubject();
        AuthenticationToken token = new APIKeyToken.Builder().apiKey("TUQOC51ZMG9CAU4").build();
        subject.login(token);
        ...
        boolean isPermitted = subject.isPermitted("questionnair:61:delete");
