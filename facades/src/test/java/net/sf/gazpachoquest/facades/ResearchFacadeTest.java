...
@ContextConfiguration(locations = { "classpath:/jpa-test-context.xml", "classpath:/datasource-test-context.xml", "classpath:/services-context.xml",
        "classpath:/components-context.xml", "classpath:/facades-context.xml" })
...
public class ResearchFacadeTest extends AbstractShiroTest {
    ...
    public void findPaginatedTest() {
        ...
        final ResearchDTO research = page.getElements().get(0);
    ...
    public void saveTest() {
        final QuestionnaireDefinitionDTO questionnaireDefinition = QuestionnaireDefinitionDTO.with().id(7).build();
    ...
    public void setUpSubject() {
        final Subject subjectUnderTest = createNiceMock(Subject.class);
        final User support = User.with().id(1).build();
    ...
    private boolean answersActivated() {
        try {
            jdbcTemplate.execute("SELECT * FROM questionnaire_answers_7");
            return true;
