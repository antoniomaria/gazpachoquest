...
public class QuestionnairDefinitionServiceTest {
	...
	public void confirmTest() {
		QuestionnairDefinition questionnairDefinition = QuestionnairDefinition
				.with().id(7).build();
	...
	public void findOneTest() {
		...
		QuestionnairDefinition questionnairDefinition = questionnairDefinitionService
	...
	public void questionGroupsCountTest() {
		int questionnairDefinitionId = 7;
		long count = questionnairDefinitionService
				.questionGroupsCount(questionnairDefinitionId);
		assertThat(count).isEqualTo(3);
	}

	@Test
	public void saveTest() {
		QuestionnairDefinitionLanguageSettings languageSettings = QuestionnairDefinitionLanguageSettings
				.with().title("My QuestionnairDefinition")
				.description("My description").build();
		QuestionnairDefinition questionnairDefinition = QuestionnairDefinition
				.with().language(Language.EN)
				.languageSettings(languageSettings).build();
		questionnairDefinition = questionnairDefinitionService
				.save(questionnairDefinition);
		DateTime lastModifiedDate = questionnairDefinition
				.getLastModifiedDate();

		QuestionnairDefinition created = questionnairDefinitionService
				.findOne(questionnairDefinition.getId());
		QuestionnairDefinitionLanguageSettings newLanguageSettings = QuestionnairDefinitionLanguageSettings
				.with().title("My QuestionnairDefinition. Ver 1")
				.description("My description").build();
		created.setLanguageSettings(newLanguageSettings);

		QuestionnairDefinition updated = questionnairDefinitionService
				.save(created);
		assertThat(lastModifiedDate)
				.isNotEqualTo(updated.getLastModifiedDate());
	}
	...
	public void saveTranslationTest() {
		...
		int questionnairDefinitionId = 7;
		QuestionnairDefinitionTranslation translation = QuestionnairDefinitionTranslation
				.with().translatedEntityId(questionnairDefinitionId)
				.language(Language.FR).languageSettings(languageSettings)
				.build();
		questionnairDefinitionService.saveTranslation(translation);
		Set<Language> translations = questionnairDefinitionService
				.translationsSupported(questionnairDefinitionId);
