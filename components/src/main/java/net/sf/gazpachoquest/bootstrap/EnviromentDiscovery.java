...
public class EnviromentDiscovery implements
		ApplicationContextInitializer<ConfigurableWebApplicationContext> {

	private static final Logger logger = LoggerFactory
			.getLogger(EnviromentDiscovery.class);
	...
	public void initialize(ConfigurableWebApplicationContext ctx) {
		...
		InputStream configurationSource = getResourceAsStream(servletContext,
				host, contextPath, CONFIG_FILE_NAME);
		...
		InputStream logbackSource = getResourceAsStream(servletContext, host,
				contextPath, "logback.xml");
		try {
			...
			throw new IllegalStateException(
					"Errors when reconfiguring logback", e);
		}
		...
		try {
			...
			environment.getPropertySources().addLast(
					new ResourcePropertySource(String.format(
							"classpath:/database/%s.properties", dbEngine)));

		} catch (IOException e) {
			throw new IllegalStateException(dbEngine
					+ ".properties not found in classpath", e);
		}
